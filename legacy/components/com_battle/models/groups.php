...
class BattleModelgroups extends JModel
{
	function get_groups()
	{
	...
	function get_group_stats()
	{
		...
		foreach ($groupstats as $group)
		{
		    ...
		    $group->faction = $this->get_faction_name($group->gid);
		}
	
	return $groupstats;
	}	
	
		function get_faction_name($gid)
	{
        $db                     = JFactory::getDBO();
        ...
        $query				    = "SELECT title FROM #__usergroups WHERE id = " . $parent;
        $db->setQuery($query);
        $groupname		        =  $db->loadResult(); 
