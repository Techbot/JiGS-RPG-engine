...
export class Part4Room extends Room<MyRoomState> {
  ...
  onCreate(options: any) {
    ...
    // set map dimensions
    Bridge.getPortals().then((result: any) => {
      this.result = result;
      return result;
      ...
      for (let i = 0; i < newResult.length; i++) {
        var newPortal = new Portal(newResult[i], this.share);
        this.world.addBody(newPortal);
