<html>
	<head>
		<script src="/components/com_battle/views/phaser/tmpl/_site/js/jquery-2.0.3.min.js"></script>
		<!-- Phaser -->
        <script src="/components/com_battle/views/phaser/tmpl/_site/js/phaser.js"></script>
		
		<style>	
			body {
				margin: 0;
			}
		</style>	
	</head>
	
	<body>
			
		<div id="world">
		</div>

    <script src='/components/com_battle/views/plate/tmpl/028_tirnanog/state01.js'></script>
	<script src='/components/com_battle/views/plate/tmpl/028_tirnanog/script.js'></script>

	</body>
</html>


