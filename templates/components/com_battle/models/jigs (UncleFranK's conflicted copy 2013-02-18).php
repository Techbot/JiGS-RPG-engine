...
class BattleModeljigs extends JModellist{
	...
	function buy_metal() {
		...
		$db->setQuery("SELECT money FROM #__jigs_players WHERE iduser =" . $user->id);
		$player_money		= $db->loadResult();
		$db->setQuery("SELECT sell_price FROM #__jigs_shop_metal_prices WHERE #__jigs_shop_metal_prices.item_id = " . $item . " AND #__jigs_shop_metal_prices.shop_id = " . $building_id);
		$sell_price = $db->loadResult();
...
<div class="gauge"><div class="xp"><span>'. $people->xp .'</span></div></div>
<div class="label">Intelligence:</div>
<div class="gauge"><div class="intel"><span>'. $people->intelligence .'</span></div></div>
<div class="label">Strength:</div>
<div class="gauge"><div class="strength"><span>'. $people->strength  .'</span></div></div>
<div class="label">Health:</div>
<div class="gauge"><div class="health"><span>'. $people->health .'</span></div></div>
</div><!-- end vitals -->
</div><!-- end profile -->
...
<div class="desc">
<img src="' . $this->baseurl. '/components/com_battle/images/ennemis/' . $people->image . '" class="thumbnail" alt="'. $people->name . ' " title="' . $people->name .'" width="100" height="100" id="character_image" />
<div class="stats">
<table class="stats" >
  <tr>
    <th scope="row">ID</th>
	...
	function attack_playa()
	{
		...
		else// roll the dice and let the games begin
		{
			...
			if ($player->dice > $player2->dice){
				...
				$message			= "You attacked " . $player2->username . " and missed. " . $player2->username . " retaliated and inflicted 10 points of damage. You: $player->health ,Opponent: $player2->health ";
